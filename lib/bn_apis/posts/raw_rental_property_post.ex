defmodule BnApis.Posts.RawRentalPropertyPost do
  use Ecto.Schema
  ...
  @junk "Junk"
  @draft "Draft"
  @schema_name "raw_rental_property_posts"

  def schema_name(), do: @schema_name

  schema "raw_rental_property_posts" do
    field :uuid, Ecto.UUID, read_after_writes: true
    field :source, :string
    field :sub_source, :string
    field :rent_expected, RoundedInteger
  ...
  @doc false
  def changeset(raw_rental_property_post, attrs \\ %{}) do
    raw_rental_property_post
  ...
  def create(params, user_map \\ %{}) do
    ...
    ch = %RawRentalPropertyPost{} |> changeset(params)
    raw_rental_property_post = ch |> Repo.insert!()
    RawPostLogs.log(@schema_name, raw_rental_property_post.id, user_map, ch.changes)

    if params["disposition"] != @draft,
      do: push_to_slash(raw_rental_property_post, user_map)

    {create_post_map(raw_rental_property_post)}
  end

  def update_post(user_map, params) do
    params = Map.put(params, "updated_by_employee_credential_id", user_map[:user_id])
    raw_rental_property_post = Repo.get_by(RawRentalPropertyPost, uuid: params["uuid"])
    ch = RawRentalPropertyPost.changeset(raw_rental_property_post, params)
    raw_rental_property_post = Repo.update!(ch)
    RawPostLogs.log(@schema_name, raw_rental_property_post.id, user_map, ch.changes)
    {create_post_map(raw_rental_property_post)}
  end
  ...
  def get_data(params) do
    ...
    query = RawRentalPropertyPost |> build_common_filter_query(params)
  ...
  def fetch_post(params) do
    ...
    if is_nil(raw_rental_property_post) do
      {:error, :not_found}
    else
      building_uuid = maybe_add_building_uuid(raw_rental_property_post)
      raw_rental_property_post = if not is_nil(building_uuid), do: Map.put(raw_rental_property_post, :building_uuid, building_uuid), else: raw_rental_property_post
      {:ok, create_post_map(raw_rental_property_post)}
    end
  end
  ...
  def push_to_slash(raw_rental_property_post, user_map) do
    lead_details = %{
      "lead_id" => raw_rental_property_post.uuid,
      "lead_type" => "rental",
      "customer_number" => raw_rental_property_post.phone,
      "city" => raw_rental_property_post.city,
      "source" => raw_rental_property_post.source,
      "table_name" => @schema_name,
      "table_uniq_identifier" => raw_rental_property_post.id
    }
  ...
  defp build_join_query(query) do
    query
    |> join(:left, [r], rpp in RentalPropertyPost, on: fragment("?::varchar = ?", r.uuid, rpp.raw_post_uuid))
  ...
  defp build_select_query(query) do
    ...
    |> select([rrpp, rpp, cc, uc], %{
      ...
      rent_expected: rrpp.rent_expected,
      name: rrpp.name,
      country_code: rrpp.country_code,
      phone: rrpp.phone,
      available_from: rrpp.available_from,
      city: rrpp.city,
      ...
      is_bachelor_allowed: rrpp.is_bachelor_allowed,
      notes: rrpp.notes,
  ...
  defp maybe_add_building_uuid(raw_rental_property_post) do
    if is_nil(raw_rental_property_post.building_uuid) && raw_rental_property_post.building && raw_rental_property_post.building != "Building Not Found" do
      case Building.get_building_by_name(raw_rental_property_post.building) do
        nil ->
          nil

        building ->
          from(r in RawRentalPropertyPost, where: r.uuid == ^raw_rental_property_post.uuid, update: [set: [building_uuid: ^building.uuid]])
          |> Repo.update_all([])
