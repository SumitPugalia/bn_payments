defmodule Mix.Tasks.UpdateReversedPayoutsFailureReason do
  use Mix.Task
  import Ecto.Query
  ...
  @shortdoc "update reversed payouts failure reason"
  def run(_) do
  ...
  defp update_payouts() do
    ...
    Payout
    |> where([p], p.status == "reversed" and is_nil(p.failure_reason))
    |> order_by([p], asc: p.id)
    |> Repo.all()
    |> Enum.each(fn payout ->
      try do
  ...
  defp update_payout(payout) do
    ...
    if not is_nil(params["failure_reason"]) do
      ch =
      ...
      IO.puts("failure_reason param is nil for payout with id: #{payout.id}")
    end
  end
